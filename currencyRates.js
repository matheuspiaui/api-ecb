...
module.exports = {
    ...
    createCurrenciesMap: function(currencies) {
      ...
      _.each(currencies, function(item) {
         ...
         self.currenciesMapCopia.push({ currency: currency, rate: rate });
      });
      self.currenciesMap.push({ currency: 'EUR', rate: 1 });
      self.currenciesMapCopia.push({ currency: 'EUR', rate: 1 });
    ...
    getRate: function(rate) {
      var self = this;
      var getCurrency = function(currency) {
        return _.find(self.currenciesMapCopia, function(item) {
           return item.currency === currency
        });
      };

      return getCurrency(rate);
    },


    converteBase: function(base){
      ...
      for (let i = 0; i < this.currenciesMap.length; i++) {        
        this.currenciesMap[i].rate = ((1 / this.currenciesMapCopia[i].rate) * _base.rate).toFixed(4);
      }
